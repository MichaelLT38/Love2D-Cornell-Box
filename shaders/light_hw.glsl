// Pass 3, ray-traced shadow variant. Identical to light.glsl -- same area
// light sampling, same BRDF, same screen-space GI gather -- except the shadow
// query: the 56-step soft-shadow march against the SDF becomes one hardware
// ray with TerminateOnFirstHit. Per-sample visibility is now binary, and the
// softness comes from where it comes from in the path tracer: the jittered
// light point, averaged by the temporal accumulator.
//
// This is the controlled-experiment discipline again: if shadows look
// different with this pass active, the intersector did it.
...
vec3 directLight(vec3 P, vec3 N, vec3 V, vec3 albedo, float rough, float metal, inout uint seed) {
	...
	if (occludedHW(P + N * 0.004, L, d - 0.02)) return vec3(0.0);
